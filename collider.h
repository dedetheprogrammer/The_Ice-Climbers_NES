...
public:
    ...
    Collider(Vector2* pos, Vector2 size, Color color = (Color){129, 242, 53, 255}) : pos(pos), size(size), color(color) {}

    void Draw() {
        //DrawRectangleLinesEx({pos->x, pos->y, size.x, size.y}, 2.0f, (Color){129, 242, 53, 255});
        DrawRectangleLinesEx({pos->x, pos->y, size.x, size.y}, 2.0f, color);
        //std::cout << "Drawing at: (" << pos->x << ", " << pos->y << ")" << std::endl;
    }
	//void Move(Vector2 position) { pos->x = position.x; pos->y = position.y; }
	void Print() { std::cout << pos->x << ", " << pos->y << std::endl;  }
};
