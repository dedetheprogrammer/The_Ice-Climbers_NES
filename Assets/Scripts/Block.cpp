...
void HoleBehavior::OnCollision(Collision contact) {
...
// ============================================================================
PopoBehavior::PopoBehavior(GameObject& gameObject, Controller& c) : Script(gameObject),
    animator(gameObject.getComponent<Animator>()),
...
PopoBehavior::PopoBehavior(GameObject& gameObject, PopoBehavior& behavior) : Script(gameObject),
    animator(gameObject.getComponent<Animator>()),
    audioplayer(gameObject.getComponent<AudioPlayer>()),
    collider(gameObject.getComponent<Collider2D>()),
    rigidbody(gameObject.getComponent<RigidBody2D>()),
...
Component* PopoBehavior::Clone(GameObject& gameObject) {
    return new PopoBehavior(gameObject, *this);
}

void PopoBehavior::OnCollision(Collision contact) {
    float deltaTime = GetFrameTime();
    int move = 0;
    if(!isSliding){
