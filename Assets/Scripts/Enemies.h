...
public:
    ...
    void OnCollision(Collision contact) {
        if (contact.gameObject.tag == "Player") {
            if (contact.gameObject.getComponent<Script, PopoBehavior>().floor_level == contact.gameObject.getComponent<Script, PopoBehavior>().last_level) {
                if (contact.contact_normal.y < 0) {
                    rigidbody.velocity = {0,0};
                    rigidbody.acceleration = {0,0};
                }
            } else if (contact.gameObject.getComponent<Script, PopoBehavior>().isAttacking) {
                ...
                if (!contact.gameObject.getComponent<Script, PopoBehavior>().isStunned) {
                    ...
                    contact.gameObject.getComponent<Script, PopoBehavior>().isStunned = true;
                    contact.gameObject.getComponent<RigidBody2D>().velocity.x = 0;
                    contact.gameObject.getComponent<Script, PopoBehavior>().lifes--;
                }
            }
        }
    }
...
private:
    ...
    bool checkPlayers() {
        for (auto& p : players) {
            if (p != nullptr &&
                (
                     p->getComponent<Script, PopoBehavior>().floor_level == floor_level || 
                    (p->getComponent<Script, PopoBehavior>().floor_level == floor_level-1 && !p->getComponent<Script, PopoBehavior>().isGrounded) ||
                    (p->getComponent<Script, PopoBehavior>().floor_level == floor_level+1 && !p->getComponent<Script, PopoBehavior>().isGrounded)
                )
            ) {
                return true;
...
public:
    ...
    void Update() override {
        ...
        if (!std::get<bool>(ini["Game"]["AdvancedAI"])) {
            ...
            for (auto& p : players) {
                if (p != nullptr) {
                    if (p->getComponent<Script, PopoBehavior>().floor_level <= 8) {
                        if (!isMoving) {
                            if (random_val() > 0.00025) {
                                if (p->getComponent<Script, PopoBehavior>().floor_level == floor_level) {
                                    move_ai();
                                    isMoving = true;
                                    animator["Walk"];
                                }
