...
private:
	...
	{
		float projection = Dot(poly.center + poly.corners[0], axis);
		float min = projection;
		float max = projection;
		for (uint32_t i = 1; i < poly.corners.size(); i++)
		{
			projection = Dot(poly.center + poly.corners[i], axis);
...
public:
	...
	{
		Vector2 clamp;
		clamp.x = (c.center.x < r.x) ?
			r.x : (c.center.x > r.x + r.width) ?
			r.x + r.width : c.center.x;

		clamp.y = (c.center.y < r.y) ?
			r.y : (c.center.y > r.y + r.height) ?
			r.y + r.height : c.center.y;

		return std::pow(c.radius, 2) >=
			std::pow(c.center.x - clamp.x, 2) +
			std::pow(c.center.y - clamp.y, 2);
	}
	...
	static bool Collides(Circle c1, Circle c2)
	{
		return std::pow(c1.radius + c2.radius, 2) >=
			std::pow(c2.center.x - c1.center.x, 2) +
			std::pow(c2.center.y - c1.center.y, 2);
	}
	static bool Collides(Polygon p, Circle c)
	{
		...
		for (auto normal : p.normals)
		{
			...
			float cProj = Dot(c.center, normal);
