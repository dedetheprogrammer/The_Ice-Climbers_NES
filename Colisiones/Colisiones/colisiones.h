...
enum ColliderType { unknown = -1, player = 0, enemy, projectile, wall, ground };
...
public:
	...
	Vector2 colNormal;	// Dirección normal al plano de colisión
	Vector2 despCol;	// Corrección para mover al objeto a una posición donde esté en espacio libre de colisiones
	float tCol;			// Tiempo transcurrido hasta la colision (en frames)
	//Ref<GameObject> parent;	// Clase padre del colisionador (para llamar a su método). La usa PhysicsManager
	std::string debugMsg = "";

	Collision() = default;
	Collision(bool v = false, ColliderType t = unknown, Vector2 norm = { 0.0,0.0 }, Vector2 desp = {0.0, 0.0}, float tiempo = 0.0f)
		: valid(v), type(t), colNormal(norm), despCol(desp), tCol(tiempo) {}
	~Collision() = default;
};
...
public:
	...
	/*
	static bool CollidesX(Rectangulo r1, Rectangulo r2)
	{
		return !(r1.x + r1.width < r2.x		// r1 is to the left of r2
			|| r2.x + r2.width < r1.x);		// r1 is to the right of r2
	}
	static bool CollidesY(Rectangulo r1, Rectangulo r2)
	{
		return !(r1.y + r1.height < r2.y	// r1 is above r2 
			|| r2.y + r2.height < r1.y);	// r1 is below r2
	}
